...
const entities: Entity[] = [
  /* ─── Column A ─── */
  {
    id: "user",
    label: "User",
    labelEn: "System Users (Admin / Staff / Member)",
    color: "#7c3aed",
    header: "#ede9fe",
    x: 30,
    y: 30,
  ...
  },
  {
    ...
    labelEn: "Lane Staff Assignments",
    color: "#6d28d9",
    header: "#ede9fe",
    x: 30,
    y: 558,
  ...
  },
  {
    id: "cardhistory",
    label: "CardHistory",
    labelEn: "Card Processing History",
    color: "#059669",
    header: "#d1fae5",
    x: 322,
    y: 522,
...
function entityHeight(e: Entity) {
  return HEADER_H + e.fields.length * ROW_H + 8;
}
...
function RelLine({ rel, emap }: { rel: Rel; emap: Record<string, Entity> }) {
  const from = emap[rel.from];
  const to = emap[rel.to];
  ...
  const my = (y1 + y2) / 2;
  const cp1x =
    rel.fromSide === "right" || rel.fromSide === "left"
      ? x1 + (x2 - x1) * 0.5
      : x1;
  const cp1y =
    rel.fromSide === "bottom" || rel.fromSide === "top"
      ? y1 + (y2 - y1) * 0.5
      : y1;
  const cp2x =
    rel.toSide === "right" || rel.toSide === "left" ? x2 - (x2 - x1) * 0.5 : x2;
  const cp2y =
    rel.toSide === "bottom" || rel.toSide === "top" ? y2 - (y2 - y1) * 0.5 : y2;
    ...
    <div className="space-y-3">
      <div className="bg-white border border-gray-200 rounded shadow-sm px-4 py-2.5 flex items-center gap-2">
        ...
        <span className="ml-auto text-xs text-gray-400">
          {entities.length} tables · {relations.length} relationships
        </span>
      </div>
      ...
      <div className="bg-white border border-gray-200 rounded shadow-sm px-4 py-2.5 flex flex-wrap gap-5 items-center">
        ...
        {[
          ["PK", "#fef08a", "#ca8a04", "#92400e", "Primary Key"],
          ["FK", "#dbeafe", "#3b82f6", "#1d4ed8", "Foreign Key"],
        ].map(([b, bg, br, tx, lbl]) => (
          <div
            key={b as string}
      ...
      <div className="grid grid-cols-3 gap-2">
        {[
          {
            col: "Column A — Users & Assignments",
            color: "#7c3aed",
            items: ["User", "Customer", "StaffAssignment"],
          },
          {
            col: "Column B — Cards & History",
            color: "#0f766e",
            items: ["CardGroup", "Card", "CardHistory"],
          },
          {
            col: "Column C — Lanes & Tickets",
            color: "#be123c",
            items: ["Lane", "Ticket", "AlertEvent"],
          },
        ].map((g) => (
          <div
            key={g.col}
            className="bg-white border border-gray-200 rounded shadow-sm px-3 py-2"
          >
            <div
            ...
            <div className="flex flex-wrap gap-1">
              {g.items.map((t) => (
                <span
                  key={t}
                  className="text-[10px] font-mono bg-gray-100 px-1.5 py-0.5 rounded text-gray-700"
                >
                  {t}
                </span>
              ))}
      ...
      <div className="bg-white border border-gray-200 rounded shadow-sm overflow-auto">
        ...
        >
          ...
          />
          {relations.map((rel, i) => (
            <RelLine key={i} rel={rel} emap={emap} />
          ))}
          {entities.map((e) => (
          ...
          {[
            {
              ...
              t: "Card",
              c: "1:N",
              d: "A card group contains many cards",
            },
            {
              f: "Customer",
              t: "Card",
              c: "1:N",
              d: "A customer owns many cards",
            },
            {
              f: "Card",
              t: "Ticket",
              c: "1:N",
              d: "A card generates many tickets",
            },
            {
              f: "Card",
              t: "CardHistory",
              c: "1:N",
              d: "A card has many history records",
            },
            {
              f: "Lane",
              t: "Ticket",
              c: "1:N",
              d: "A lane processes many tickets",
            },
            {
              f: "User",
              t: "Ticket",
              c: "1:N",
              d: "A staff handles many tickets",
            },
            {
              f: "User",
              t: "StaffAssignment",
              c: "1:N",
              d: "A staff has many assignments",
            },
            {
              f: "Lane",
              t: "StaffAssignment",
              c: "1:N",
              d: "A lane has many assignments",
            },
            {
              f: "User",
              t: "CardHistory",
              c: "1:N",
              d: "A staff processes many card actions",
            },
            {
              f: "Lane",
              t: "AlertEvent",
              c: "1:N",
              d: "A lane generates many alerts",
            },
          ].map((r) => (
            <div
              key={r.f + r.t}
