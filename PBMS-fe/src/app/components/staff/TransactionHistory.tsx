  ...
  const handleSearch = () => {
    ...
    if (maVe.trim()) {
      ...
      result = result.filter(
        (t) => t.maVe && t.maVe.toLowerCase().includes(term),
      );
    }
        ...
        <div className="overflow-x-auto">
          {loading ? (
            <div className="flex justify-center items-center py-16 text-sm text-gray-400">
              <div className="animate-spin mr-2">⏳</div> Đang tải lịch sử giao
              dịch...
            </div>
          ) : (
              ...
              <tbody>
                {pageData.length === 0 ? (
                  ...
                  pageData.map((row, i) => (
                    <tr
                      key={row.id}
                      className={`border-b border-gray-100 hover:bg-blue-50/50 ${i % 2 === 1 ? "bg-gray-50/30" : "bg-white"}`}
                    >
                      ...
                      <td className="px-3 py-2 text-xs font-bold text-gray-800 uppercase">
                        {row.bienSo}
                      </td>
                      ...
                      <td className="px-3 py-2">
                        <span
                          className={`inline-flex px-1.5 py-0.5 rounded text-xs font-medium ${row.loaiXe === "Xe máy" ? "bg-blue-100 text-blue-700" : "bg-amber-100 text-amber-700"}`}
                        >
                          {row.loaiXe}
                        </span>
                      </td>
                      <td className="px-3 py-2">
                        <span
                          className={`inline-flex px-1.5 py-0.5 rounded text-xs font-medium ${row.loaiVe === "Vé tháng" ? "bg-green-100 text-green-700" : "bg-gray-100 text-gray-600"}`}
                        >
                          {row.loaiVe}
                        </span>
                      </td>
                      <td className="px-3 py-2 text-xs text-gray-600 tabular-nums">
                        {formatDateTime(row.tgVao)}
                      </td>
                      <td className="px-3 py-2 text-xs text-gray-600 tabular-nums">
                        {formatDateTime(row.tgRa)}
                      </td>
                      <td className="px-3 py-2 text-xs font-medium text-gray-700">
                        {row.phi !== null && row.phi !== undefined
                          ? `${row.phi.toLocaleString("vi-VN")} VNĐ`
                          : "—"}
                      </td>
                      <td className="px-3 py-2">
                        <span
        ...
        {totalPages > 1 && (
          <div className="px-4 py-2.5 border-t border-gray-200 flex items-center justify-between">
            <span className="text-xs text-gray-500">
              Hiển thị {Math.min((page - 1) * PAGE_SIZE + 1, filtered.length)}–
