...
type VStatus =
  "Pending Approval" | "Approved-Unpaid" | "Paid" | "Waived" | "Rejected";
...
export default function StaffViolations() {
  ...
  const handleSubmit = () => {
    ...
    if (!form.vehiclePlate.trim())
      return setFormError("Biển số xe là bắt buộc.");
    if (!form.slotCode.trim()) return setFormError("Mã slot là bắt buộc.");
    if (!form.amount || isNaN(Number(form.amount)))
      return setFormError("Số tiền phạt phải là số hợp lệ.");
  ...
  return (
    <div className={cls.pageWrapper}>
      ...
      <div className="flex gap-0 mb-3 border-b border-gray-200">
        {[
          { key: "create", label: "Tạo vi phạm" },
          { key: "reports", label: "Báo cáo của tôi" },
        ].map((t) => (
          <button
            key={t.key}
            onClick={() => {
              setTab(t.key as "create" | "reports");
              setSubmitDone(null);
            }}
          ...
          {submitDone ? (
            <div className={`${cls.sectionCard} p-6 text-center`}>
              ...
              <p className="text-sm text-gray-500 mb-1">
                Mã vi phạm: <strong>{submitDone.id}</strong>
              </p>
              <p className="text-sm text-gray-500 mb-4">
                Trạng thái:{" "}
                <span className={cls.badge.amber}>Pending Approval</span>
              </p>
              <button
            ...
            <div className={`${cls.sectionCard} p-4`}>
              <h2 className="text-sm font-semibold text-gray-700 border-b pb-2 mb-3">
                Thông tin vi phạm
              </h2>
              <div className="space-y-3">
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setForm((p) => ({ ...p, vehiclePlate: e.target.value }))
                      }
                    />
                  </div>
                  <div>
                    ...
                    <input
                      ...
                      value={form.slotCode}
                      onChange={(e) =>
                        setForm((p) => ({ ...p, slotCode: e.target.value }))
                      }
                    />
                  </div>
                </div>
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div>
                    <label className="block text-xs text-gray-500 mb-0.5">
                      Thẻ tháng (không bắt buộc)
                    </label>
                    <input
                      className={`${cls.input} w-full`}
                      placeholder="MC-C-001"
                      value={form.monthlyCard}
                      onChange={(e) =>
                        setForm((p) => ({ ...p, monthlyCard: e.target.value }))
                      }
                    />
                  </div>
                </div>
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div>
                    <label className="block text-xs text-gray-500 mb-0.5">
                      Số tiền phạt (VNĐ) <span className="text-red-500">*</span>
                    </label>
                    <input
                      type="number"
                      className={`${cls.input} w-full`}
                ...
                <div>
                  <label className="block text-xs text-gray-500 mb-0.5">
                    Request liên quan (không bắt buộc)
                  </label>
                  <input
                    className={`${cls.input} w-full`}
                    placeholder="REQ-001"
