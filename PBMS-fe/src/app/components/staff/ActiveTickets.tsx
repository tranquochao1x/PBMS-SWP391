  ...
  const handleSearch = () => {
    setFiltered(
      activeTickets.filter((t) => {
        ...
        const matchMaVe =
          !maVe || t.maVe.toLowerCase().includes(maVe.toLowerCase());
    ...
    <div className="space-y-3">
      ...
      <div className="bg-white border border-gray-200 rounded shadow-sm overflow-hidden">
            ...
            <tbody>
              {filtered.length === 0 ? (
                ...
                filtered.map((row, i) => (
                  <tr
                    key={row.id}
                    className={`border-b border-gray-100 hover:bg-blue-50 ${i % 2 === 1 ? "bg-gray-50/50" : "bg-white"}`}
                  >
        ...
        <div className="px-4 py-2 border-t border-gray-200 flex items-center justify-between">
          <span className="text-xs text-gray-500">
            Tổng: {filtered.length} vé đang hoạt động
          </span>
        </div>
      </div>
      ...
      {detail && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="w-[420px] overflow-hidden rounded-lg bg-white shadow-xl">
            {/* Header */}
            <div className="flex items-center justify-between bg-blue-600 px-5 py-3">
              <span className="text-sm font-semibold text-white">
                Chi tiết vé — {detail.maVe}
              </span>
            ...
            <div className="p-5">
              <div className="space-y-3.5">
                ...
                <div>
                  ...
                  <span className="mt-0.5 inline-flex rounded border border-green-200 bg-green-100 px-2 py-0.5 text-xs font-bold tracking-wide text-green-700">
                    {detail.trangThai}
                  </span>
                </div>
              </div>
