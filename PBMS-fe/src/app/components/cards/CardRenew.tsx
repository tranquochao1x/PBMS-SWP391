...
export default function CardRenew() {
  ...
  const price = selected
    ? (priceMap[selected.nhomThe]?.[renewMonths] ?? "---")
    : "";

  const handleRenew = () => {
    setSuccessMsg(
      `Gia hạn thẻ ${selected?.maThe} thành công! Ngày hết hạn mới: ${newExpiry}`,
    );
            ...
            {!selected ? (
              <div className="p-6 text-center">
                ...
                <p className="text-xs text-gray-400">
                  Chọn thẻ cần gia hạn từ danh sách bên trái
                </p>
              </div>
            ) : (
              <div className="p-3 space-y-3">
                  ...
                  <div className="flex justify-between text-xs pt-1 border-t border-blue-200 mt-1">
                    ...
                    <span
                      className={`font-medium ${
                        selected.trangThai === "Hết hạn"
                          ? "text-red-600"
                          : selected.trangThai === "Sắp hết hạn"
                            ? "text-amber-600"
                            : "text-gray-800"
                      }`}
                    >
                ...
                <div className="flex gap-2">
                  <button
                    className={`${cls.btnSearch} flex-1 justify-center`}
                    onClick={handleRenew}
                  >
