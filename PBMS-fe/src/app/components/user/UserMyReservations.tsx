...
const statusBadge: Record<ResStatus, string> = {
  ...
  "No-show":
    "inline-flex items-center px-1.5 py-0.5 rounded text-xs font-medium bg-red-200 text-red-900",
};
...
function canCancel(res: Reservation): boolean {
  ...
  return (
    (res.status === "Confirmed" ||
      res.status === "Awaiting User Confirmation") &&
    diffH >= 6
  );
}
...
export default function UserMyReservations() {
  ...
  const filtered = data.filter((r) => {
    ...
    if (filterSlot && !r.slot.toLowerCase().includes(filterSlot.toLowerCase()))
      return false;
  ...
  const handleCancel = (id: string) => {
    setData((prev) =>
      prev.map((r) =>
        r.id === id ? { ...r, status: "Cancelled by User" as ResStatus } : r,
      ),
    );
  };
      ...
      <div className={`${cls.filterSection} flex flex-wrap gap-2 items-end`}>
        ...
        <div>
          ...
          >
            ...
            {(Object.keys(statusBadge) as ResStatus[]).map((s) => (
                  ...
                  <td className={cls.td}>
                    <div className="flex gap-1">
                      ...
                      <button
                        ...
                        title={
                          !canCancel(r)
                            ? "Không thể hủy (< 6 giờ trước giờ đến hoặc trạng thái không hợp lệ)"
                            : ""
                        }
                      >
                        <X className="w-3 h-3" />
