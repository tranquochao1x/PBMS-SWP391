...
function AddCardModal({
  ...
  const [form, setForm] = useState(() => {
    const defaultGroup =
      cardGroups.find((g) => g.groupName === "THẺ THÁNG XE MÁY") ||
      cardGroups[0];
  ...
  const loaiXe = selectedGroup
    ? selectedGroup.vehicleType === "CAR"
      ? "Ô tô"
      : "Xe máy"
    : "Xe máy";
  const today = getTodayDate();
  const ngayHetHan = isDayCard
    ? addDays(form.ngayDangKy, duration)
    : addMonths(form.ngayDangKy, duration);
    ...
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg shadow-xl w-[500px] overflow-hidden">
        ...
        <div className="p-5 space-y-3">
          ...
          <div>
            <label className="block text-xs text-gray-600 mb-1">Nhóm thẻ</label>
            <select
              ...
              onChange={(e) => {
                ...
                const selected = cardGroups.find(
                  (g) => g.groupName === nhomTheMoi,
                );
                const isOtoMoi = selected
            ...
            <div className="mt-1.5 rounded border border-blue-100 bg-blue-50 px-3 py-2 text-xs text-blue-700">
              Loại xe tự động: <span className="font-semibold">{loaiXe}</span>
            </div>
          </div>
            ...
            {noMatchingVehicle ? (
              <>
                ...
                <p className="mt-1.5 text-xs text-red-600 bg-red-50 border border-red-200 rounded px-2 py-1.5">
                  Bạn chưa đăng ký phương tiện phù hợp cho nhóm thẻ này. Vui
                  lòng vào mục 'Phương tiện của tôi' để thêm xe trước!
                </p>
              </>
            ) : (
              <select
          ...
          <div className="bg-blue-50 border border-blue-200 rounded px-3 py-2.5 space-y-1">
            <div className="flex justify-between text-xs text-blue-700">
              ...
              <span className="font-semibold">{ngayHetHan}</span>
            </div>
            <div className="flex justify-between items-center">
              <span className="text-xs text-blue-700">
                Tổng phí ({duration} {isDayCard ? "ngày" : "tháng"}):
              </span>
        ...
        <div className="flex justify-end gap-2 px-5 py-3 border-t border-gray-200">
          <button
            onClick={handleNext}
            disabled={noMatchingVehicle}
            className="flex items-center gap-1.5 h-[34px] px-4 bg-blue-600 hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed text-white text-sm font-medium rounded transition-colors"
          >
...
function RenewModal({
  ...
  const isDayCard = selectedGroup
    ? selectedGroup.ticketType === "DAY"
    : card.nhomThe.includes("NGÀY");
        ...
        <div className="p-5 space-y-4">
          <div className="bg-gray-50 border border-gray-200 rounded p-3 space-y-2">
            ...
            <div className="flex justify-between gap-4">
              ...
              <span
                className={`text-xs font-semibold ${cardIsExpired ? "text-red-600" : "text-gray-800"}`}
              >
                {card.ngayHetHan}
              </span>
            </div>
          ...
          <div>
            <label className="block text-xs font-medium text-gray-600 mb-1">
              {isDayCard ? "Số ngày gia hạn" : "Số tháng gia hạn"}
            </label>
            <select
          ...
          <div className="bg-emerald-600 rounded-lg p-4">
            <div className="flex justify-between text-emerald-100 text-xs mb-1">
              ...
              <span className="font-semibold text-white">{newExpiry}</span>
            </div>
          </div>
        </div>
    ...
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg shadow-xl w-[420px] overflow-hidden">
        ...
        <div className="flex justify-end px-5 py-3 border-t border-gray-200">
          <button
            onClick={onClose}
            className="h-[34px] px-4 border border-gray-300 text-gray-600 hover:bg-gray-50 text-sm rounded transition-colors"
          >
            Đóng
          </button>
        </div>
...
function PaymentQrModal({
  ...
  const handleCancel = async () => {
    ...
    if (window.confirm("Ban co chac chan muon huy thanh toan nay khong?")) {
      try {
        await cardService.cancelPayment(
          orderCode,
          "Nguoi dung chu dong huy tren giao dien",
        );
  ...
  return (
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-[60]">
      <div className="bg-white rounded-lg shadow-xl w-[400px] overflow-hidden text-center">
        {/* Header */}
        <div
          className={`flex items-center justify-between px-5 py-3 ${isPaid ? "bg-emerald-600" : "bg-blue-600"}`}
        >
          ...
          {isPaid ? (
            ...
            <>
              <p className="text-sm text-gray-600">
                Quét mã QR dưới đây bằng ứng dụng ngân hàng hoặc ví điện tử để
                thanh toán
              </p>
...
export default function UserMonthlyCards() {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(async () => {
      pollCount++;
      try {
        const statusData = await cardService.checkPaymentStatus(
          pendingConfirmation.orderCode,
        );
        ...
        if (statusData && statusData.status === "PAID") {
          ...
          statusData &&
          (statusData.status === "CANCELLED" || statusData.status === "FAILED")
        ) {
          ...
          setPendingConfirmation(null);
          alert(
            "⚠️ Giao dịch đã hết thời gian chờ hoặc bị hủy bỏ!\nVui lòng kiểm tra lại trạng thái thẻ.",
          );
  ...
  const handleRenew = async (id: number, newExpiry: string) => {
    ...
    try {
      ...
      if (updatedCard.qrCode && updatedCard.checkoutUrl) {
        ...
        setCards((prev) =>
          prev.map((c) => (c.id === id ? refreshCardStatus(updatedCard) : c)),
        );
        setRenewCard(null);
      }
    } catch (err: any) {
    ...
    <div className="space-y-3">
      <div className="bg-white border border-gray-200 rounded shadow-sm px-4 py-2.5 flex items-center justify-between">
        ...
        <button
          ...
          className="flex items-center gap-1.5 h-[34px] px-3 bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium rounded transition-colors"
        >
      ...
      <div className="grid grid-cols-3 gap-3">
        {[
          ...
          <div
            key={s.label}
            className={`${s.bg} rounded shadow-sm border border-gray-200 px-4 py-3`}
          >
            ...
            >
              ...
              >
                <div>
                  ...
                  <span
                    className={`inline-flex px-1.5 py-0.5 rounded text-xs font-medium ${card.loaiXe === "Xe máy" ? "bg-blue-100 text-blue-700" : "bg-amber-100 text-amber-700"}`}
                  >
                    {card.loaiXe}
                  </span>
                </div>
              ...
              <div className="px-4 py-2.5 border-t border-gray-100 flex gap-2">
                <button
                  ...
                  className="flex items-center gap-1 text-xs text-blue-600 hover:text-blue-800 border border-blue-200 hover:border-blue-400 px-2.5 py-1 rounded transition-colors"
                >
                ...
                <button
                  onClick={() => handleOpenRenewModal(card)}
                  className="flex items-center gap-1 text-xs text-emerald-600 hover:text-emerald-800 border border-emerald-200 hover:border-emerald-400 px-2.5 py-1 rounded transition-colors"
                >
