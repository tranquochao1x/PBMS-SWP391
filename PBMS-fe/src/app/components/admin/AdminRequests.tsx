...
export default function AdminRequests() {
  ...
  const [approveItem, setApproveItem] = useState<RequestSupportDto | null>(
    null,
  );
  ...
  const handleApprove = async () => {
    ...
    try {
      setError("");
      const updated = await adminCardService.approveRequest(
        approveItem.requestId,
        approveNote.trim(),
      );
      setRequests((prev) =>
  ...
  const handleReject = async () => {
    ...
    try {
      setError("");
      const updated = await adminCardService.rejectRequest(
        rejectItem.requestId,
        rejectReason.trim(),
      );
      setRequests((prev) =>
        prev.map((r) => (r.requestId === updated.requestId ? updated : r)),
  ...
  const handleAssign = async () => {
    ...
    try {
      setError("");
      const updated = await adminCardService.assignRequestStaff(
        assignItem.requestId,
        Number(assignStaffId),
      );
      setRequests((prev) =>
        prev.map((r) => (r.requestId === updated.requestId ? updated : r)),
      );
    ...
    <div className={cls.pageWrapper}>
      <div className="flex items-center gap-2 mb-3">
        ...
        <h1 className="text-base font-semibold text-gray-800">
          Yêu cầu hỗ trợ từ Staff & Khách hàng
        </h1>
      </div>

      {error && (
        <div className="mb-3 text-sm text-red-600 bg-red-50 border border-red-200 rounded p-2.5">
          {error}
        </div>
      )}
            ...
            <tbody>
              {loading ? (
                ...
                filtered.map((r, i) => (
                  <tr
                    key={r.requestId}
                    className={`border-b border-gray-100 ${i % 2 === 0 ? "bg-white" : "bg-gray-50/50"} hover:bg-gray-100/30 transition-colors`}
                  >
                    <td className={cls.td}>
                    ...
                    <td className="px-2 py-1.5 text-sm border-r border-gray-200 text-gray-800 break-words">
                      {r.subject}
                    </td>

                    <td className={cls.td}>{r.senderName}</td>
                    <td className={cls.td}>
                      <span className={statusBadge[r.status] || cls.badge.gray}>
                        {mapStatusToVi(r.status)}
                      </span>
                    </td>
                    <td className={cls.td}>
                      {r.createdAt
                    ...
                    <td className={cls.td}>
                      <div className="flex gap-1 flex-wrap">
                        <button
                          ...
                          className="h-6 px-2 bg-blue-50 hover:bg-blue-100 text-blue-700 text-xs rounded border border-blue-200 flex items-center gap-1"
                        >
                        ...
                        {(r.status === "PENDING" ||
                          ...
                          <button
                            ...
                            className="h-6 px-2 bg-green-50 hover:bg-green-100 text-green-700 text-xs rounded border border-green-200 flex items-center gap-1"
                          >
                        ...
                        {r.status !== "RESOLVED" &&
                          ...
                          r.status !== "REJECTED" && (
                            <button
                              ...
                              className="h-6 px-2 bg-red-50 hover:bg-red-100 text-red-700 text-xs rounded border border-red-200 flex items-center gap-1"
                            >
                        ...
                        {r.status !== "RESOLVED" &&
                          ...
                          r.status !== "REJECTED" && (
                            <button
                              onClick={() => {
                                setAssignItem(r);
                                setAssignStaffId(r.assignedStaffId || "");
                              }}
                              className="h-6 px-2 bg-purple-50 hover:bg-purple-100 text-purple-700 text-xs rounded border border-purple-200 flex items-center gap-1"
                            >
          ...
          <div className="bg-white rounded-lg shadow-xl w-full max-w-md p-5 border border-gray-150">
            <div className="flex justify-between items-center mb-4 pb-2 border-b border-gray-100">
              <h3 className="font-semibold text-gray-700">
                Chi tiết yêu cầu – {viewItem.requestNo}
              </h3>
            ...
            <div className="space-y-2 text-sm max-h-[60vh] overflow-y-auto pr-1">
              ...
              <div className="flex justify-between border-b border-gray-100 pb-1">
                ...
                <span
                  className={statusBadge[viewItem.status] || cls.badge.gray}
                >
                  {mapStatusToVi(viewItem.status)}
                </span>
              </div>
              <div className="flex justify-between border-b border-gray-100 pb-1">
                ...
                <span>
                  {viewItem.createdAt
                    ? new Date(viewItem.createdAt).toLocaleString("vi-VN")
                    : "—"}
                </span>
              </div>
              ...
              <div className="flex justify-between border-b border-gray-100 pb-1">
                ...
                <span>
                  {viewItem.processingAt
                    ? new Date(viewItem.processingAt).toLocaleString("vi-VN")
                    : "—"}
                </span>
              </div>
              <div className="flex justify-between border-b border-gray-100 pb-1">
                ...
                <span>
                  {viewItem.resolvedAt
                    ? new Date(viewItem.resolvedAt).toLocaleString("vi-VN")
                    : "—"}
                </span>
              </div>
          ...
          <div className="bg-white rounded-lg shadow-xl w-full max-w-sm p-5 border border-gray-150">
            ...
            <div className="flex justify-end gap-2 mt-4">
              ...
              <button
                ...
                className="h-[34px] px-4 bg-emerald-600 hover:bg-emerald-700 text-white text-sm rounded flex items-center gap-1.5 cursor-pointer"
              >
          ...
          <div className="bg-white rounded-lg shadow-xl w-full max-w-sm p-5 border border-gray-150">
            ...
            <div className="flex justify-end gap-2 mt-4">
              ...
              <button
                ...
                className={`${cls.btnDanger} ${!rejectReason.trim() ? "opacity-50 cursor-not-allowed" : ""}`}
              >
                Từ chối
              </button>
            </div>
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg shadow-xl w-full max-w-sm p-5 border border-gray-150">
            ...
            <select
              ...
              value={assignStaffId}
              onChange={(e) =>
                setAssignStaffId(e.target.value ? Number(e.target.value) : "")
              }
            >
            ...
            <div className="flex justify-end gap-2 mt-4">
              ...
              <button
                ...
                className={`${cls.btnSearch} ${!assignStaffId ? "opacity-50 cursor-not-allowed" : ""}`}
              >
