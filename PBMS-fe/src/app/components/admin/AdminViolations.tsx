...
const SAMPLE: Violation[] = [
  ...
  },
  {
    ...
    user: "Trần Thị B",
    monthlyCard: "MC-C-002",
    slot: "B2-B01",
    type: "Overnight",
    amount: 500000,
    evidence: "Camera B2 23:00",
    relatedRequestId: "",
    createdBy: "staff02",
  ...
  },
  {
    ...
    user: "Phạm Thị D",
    monthlyCard: "MC-C-004",
    slot: "B2-A03",
    type: "Wrong Slot",
    amount: 200000,
    evidence: "Camera B2 09:10",
  ...
  {
    id: "VIO-006",
    vehiclePlate: "51B-44444",
    user: "Đỗ Thị F",
    monthlyCard: "MC-C-006",
    slot: "B2-B02",
    type: "Overtime",
    amount: 150000,
    evidence: "Camera B2 19:00",
    relatedRequestId: "",
    createdBy: "staff02",
    status: "Rejected",
    date: "2026-06-08",
...
const statusBadge: Record<VStatus, string> = {
  ...
  Waived:
    "inline-flex items-center px-1.5 py-0.5 rounded text-xs font-medium bg-teal-100 text-teal-700",
  "Refund Pending": cls.badge.amber,
  "Refund Disputed":
    "inline-flex items-center px-1.5 py-0.5 rounded text-xs font-medium bg-orange-100 text-orange-700",
...
export default function AdminViolations() {
  ...
  const doApprove = () => {
    ...
    setData((prev) =>
      prev.map((v) =>
        v.id === approveItem.id
          ? { ...v, status: "Approved-Unpaid" as VStatus }
          : v,
      ),
    );
    setApproveItem(null);
  ...
  const doReject = () => {
    ...
    setData((prev) =>
      prev.map((v) =>
        v.id === rejectItem.id ? { ...v, status: "Rejected" as VStatus } : v,
      ),
    );
    setRejectItem(null);
    setRejectReason("");
      ...
      <div className={`${cls.filterSection} flex flex-wrap gap-2 items-end`}>
        <div>
          ...
          >
            ...
            {(Object.keys(statusBadge) as VStatus[]).map((s) => (
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg shadow-xl w-full max-w-lg p-5">
            ...
            <div className="grid grid-cols-2 gap-2 text-sm">
              {[
                ["ID", viewItem.id],
                ["Biển số", viewItem.vehiclePlate],
                ["Người dùng", viewItem.user],
                ["Thẻ tháng", viewItem.monthlyCard],
                ["Slot", viewItem.slot],
                ["Loại", viewItem.type],
                ["Số tiền", `${viewItem.amount.toLocaleString("vi-VN")} VNĐ`],
                ["Bằng chứng", viewItem.evidence],
                ["Req liên quan", viewItem.relatedRequestId || "—"],
                ["Tạo bởi", viewItem.createdBy],
                ["Ngày", viewItem.date],
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg shadow-xl w-full max-w-sm p-5">
            ...
            <p className="text-sm text-gray-600 mb-4">
              Xác nhận duyệt vi phạm <strong>{approveItem.id}</strong>? Trạng
              thái sẽ chuyển thành{" "}
              <span className={cls.badge.red}>Approved-Unpaid</span>.
            </p>
            <div className="flex justify-end gap-2">
              ...
              <button onClick={doApprove} className={cls.btnSearch}>
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg shadow-xl w-full max-w-sm p-5">
            ...
            <p className="text-sm text-gray-600 mb-3">
              Vi phạm: <strong>{rejectItem.id}</strong>
            </p>
            <div>
              <label className="block text-xs text-gray-500 mb-0.5">
